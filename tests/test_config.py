...
def test_load_minimal_config_uses_defaults(minimal_config_file: Path):
    cfg = load_config(minimal_config_file)
    assert cfg.poll_interval == 300
    assert cfg.repos == []


def test_config_path_as_string(config_file: Path):
...
def test_env_github_token_overrides_file(config_file: Path, monkeypatch):
    monkeypatch.setenv("GITHUB_TOKEN", "ghp_from_env")
    cfg = load_config(config_file)
    assert cfg.github_token == "ghp_from_env"


def test_env_github_monitor_config_path(config_file: Path, monkeypatch):
...
def test_env_token_provides_missing_token(tmp_path: Path, monkeypatch):
...
def test_missing_token(tmp_path: Path):
    p = tmp_path / "config.toml"
    p.write_text('github_username = "testuser"\n')
...
def test_poll_interval_too_low(tmp_path: Path):
    p = tmp_path / "config.toml"
    p.write_text(
...
def test_poll_interval_wrong_type(tmp_path: Path):
    p = tmp_path / "config.toml"
    p.write_text(
        'github_token = "ghp_abc"\ngithub_username = "user"\npoll_interval = "fast"\n'
    )
...
def test_invalid_repo_format(tmp_path: Path):
    p = tmp_path / "config.toml"
    p.write_text(
        'github_token = "ghp_abc"\n'
        'github_username = "user"\n'
...
def test_repos_not_a_list(tmp_path: Path):
    p = tmp_path / "config.toml"
    p.write_text(
        'github_token = "ghp_abc"\ngithub_username = "user"\nrepos = "owner/repo"\n'
    )
...
def test_empty_token_string(tmp_path: Path):
    p = tmp_path / "config.toml"
...
def test_empty_username_string(tmp_path: Path):
    p = tmp_path / "config.toml"
    p.write_text('github_token = "ghp_abc"\ngithub_username = ""\n')
    with pytest.raises(ConfigError, match="github_username is required"):
        load_config(p)
...
def test_poll_interval_at_boundary(tmp_path: Path):
