...
class TestOpenUrlPortal:
    ...
    async def test_portal_success(self) -> None:
        ...
        with patch("github_monitor.notifier.MessageBus", return_value=bus):
...
class TestOpenUrl:
    ...
    async def test_uses_portal_when_available(self) -> None:
        ...
        with (
            patch("github_monitor.notifier._open_url_portal", new_callable=AsyncMock, return_value=True) as mock_portal,
            patch("github_monitor.notifier._open_url_xdg", new_callable=AsyncMock) as mock_xdg,
        ):
            await _open_url("https://github.com/owner/repo/pull/1")
    ...
    async def test_falls_back_to_xdg_open(self) -> None:
        ...
        with (
            patch(
                "github_monitor.notifier._open_url_portal", new_callable=AsyncMock, return_value=False
            ) as mock_portal,
            patch("github_monitor.notifier._open_url_xdg", new_callable=AsyncMock) as mock_xdg,
        ):
            await _open_url("https://github.com/owner/repo/pull/1")
